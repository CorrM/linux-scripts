...
xrandr_command="xrandr --output \"$primary_monitor\" --primary --mode ${res_rate%@*} --rate ${res_rate#*@}"

# Prepare the commands to turn off other monitors
turn_off_commands=""
for monitor in "${connected_monitors[@]}"; do
    if [[ "$monitor" != "$primary_monitor" ]]; then
        turn_off_commands+="xrandr --output \"$monitor\" --off\n"
    fi
done
...
awk -v cmd="$xrandr_command" -v turn_off="$turn_off_commands" '
    BEGIN {inserted=0}
    /^#/ {print; next}
    !inserted {print cmd; print turn_off; inserted=1}
    {print}
